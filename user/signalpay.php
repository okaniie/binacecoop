...
if (isset($_GET['signal'])) {
  $signal = $_GET['signal'];
  $sw = mysqli_query($link, "SELECT * FROM package1 WHERE id = '$signal' ");
  if (mysqli_num_rows($sw) > 0) {
    $sig = mysqli_fetch_assoc($sw);
    $min_amt = $sig['froms'];
  }
}else{
  echo "<script>window.location.href = 'signal.php' </script>";
}
...
if (isset($_POST['Send'])) {
  ...
  if (!empty($method) && !empty($amount)) {
    if ($amount < $min_amt) {
      echo "<script>alert('The minimum amount to deposit is ".$min_amt." ');window.location.href = 'window.location.href' </script>";
    }else{
      $status = "Pending";
      $currency1 = "USD";
      $insert = mysqli_query($link, "INSERT INTO deposits ( `method`, `package`, `email`, `amount`, `currency`, `status`) VALUES ('$method', '$signal', '$email', '$amount', '$currency1', '$status') ");
      if ($insert) {
        $subject = "New Deposit";
        $body = "<p>Dear ".$fname." </p> <p>You initiated a deposit of ".$amount." USD of ".$method." </p> <p>You trading accout will be credit once it is approved</p> <p>Thank You</p> ";
          ...
          <div class="card-body pb-0 mx-25">
            ...
            <div class="row my-3">
              ...
              <div class="col-12 d-flex justify-content-end">
                <img src="payment.png" alt="logo" height="60" width="224"  >
              </div>
            </div>
            <hr>
            ...
            <div class="row invoice-info">
              <div class="col-6 mt-1">
                <h6 class="invoice-from" style="color:#000;font-size:20px;font-weight:bolder">Signal Payment  </h6>
                <div class="mb-1">
              ...
              <div class="col-6 mt-1">
              <h6 class="invoice-from" style="color:#000;font-size:20px;font-weight:bolder"> </h6>
                <div class="mb-1">
            ...
            <hr>
                ...
                <div class="col-sm-12" style="background-color:#000;font-weight:bolder;padding:10px;color:#fff">
				  ...
				  Kindly Make A Payment Of <b style="color:#4da9ff">$<?php echo $min_amt ?> above</b> To The Bitcoin Address Below.
				  </center>
            ...
            <div class="row">
              <div class="col-12 col-sm-12 mt-75" style="background-color:#000;padding:10px;border-radius:4px">
                <p style="text-align:left;color:#fff;float:left">Sign this contract by copying and pasting the hash generated by this transaction in the field below
					<form method="POST">	
					<input type='text' class='form-control' min="<?php echo $min_amt ?>" name='amount' style='float:left;background-color:#fff;margin-top:5px;border:none' placeholder='Enter Amount Sent in <?php echo $currency ?>' value='<?php echo $min_amt ?>' >
					<br>
